...
\title{Local number variance as a causal handle in the one-dimensional Bose--Hubbard chain}
...
causally effective intervention sites in a one-dimensional open
Bose--Hubbard chain under Lindblad dephasing.
...
The primary sweep reveals three regimes: at $J/U=0.12$ targeted intervention
is \emph{harmful} (95\% CI strictly below zero at all tested $L$ and $\tau$);
...
near-optimal.
Taken together, local number variance identifies causally effective
intervention sites in the positive pocket ($J/U\geq0.30$), and this
advantage is not reducible to geometric centrality, shell position, mean
occupation, or disorder amplitude, while surviving the tested deterministic
...
at $L=6$.
At $J/U=0.12$ the targeted intervention is \emph{harmful}: the 95\% CI is
strictly below zero at all tested $L$ and $\tau$.
...
$J/U=0.12$ is uniformly negative (negative regime); $J/U=0.20$ is
...
all three tested chain lengths and all horizons.
...
$J/U=0.40$, $\tau=3$: local variance (fi), geometric centrality (geo),
maximum mean occupation (maxn), minimum mean occupation (minn), boundary
distance (bdy), anti-variance (anti), generator action (gen), disorder
amplitude (dis\_amp), and anti-disorder-amplitude (dis\_anti).
...
At $J/U=0.40$, $\tau=3$: fi$>$geo with a gap of $+0.032$ to $+0.054$
...
variance $\Fi = \langle\nop_i^2\rangle - \langle\nop_i\rangle^2$
identifies causally effective intervention sites in the positive pocket
$J/U\geq0.30$: targeting the highest-$\Fi$ sites with additional dephasing
produces more local future occupation loss than matched-budget random
targeting, robustly across all tested $L\in\{6,7,8\}$ and
$\tau\in\{1,2,3\}$, with the effect increasing with system size.
The handle is regime-dependent---harmful at $J/U=0.12$, transitional near
$J/U\approx0.20$---and its advantage is not reducible to geometric
centrality, shell position, mean occupation, or disorder amplitude.

\paragraph*{Mechanism.}
The site-resolved response is consistent with nonlocal redistribution
rather than purely local depletion: additional dephasing at high-$\Fi$
sites alters the subsequent spatial pattern of occupation loss across the
chain.
The present paper does not attempt a full microscopic transport account;
its main result is the regime-dependent identification of high-$\Fi$ sites
as effective causal handles.
...
or all system sizes; that the mechanism generalises to other quantum
systems; or that the occupation-variance handle corresponds to any
equilibrium observable.
No thermodynamic-limit or universal-handle claims are made.
