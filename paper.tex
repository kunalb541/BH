...
\begin{abstract}
We show that sites of high local occupation variance
$F_i = \langle\nop_i^2\rangle - \langle\nop_i\rangle^2$
function as causal handles on future local occupation loss in an open
one-dimensional Bose--Hubbard chain under Lindblad dephasing.
Applying additional dephasing at the top-$k$ highest-$F_i$ sites produces
more local occupation loss than an equal-budget random targeting
at $J/U\in\{0.30,0.40\}$, with 95\% bootstrap confidence intervals
excluding zero across all three tested time horizons ($\tau\in\{1,2,3\}$)
and both tested chain lengths ($L\in\{6,7\}$).
The effect is absent or reversed at the lowest tested coupling
($J/U=0.12$) and size-sensitive at $J/U=0.20$; the handle is a
regime-specific property of the dephasing operator, not a generic one.
The site-resolved spatial response is more consistent with nonlocal
redistribution of occupation change than with a local-instability
mechanism.
All results use exact Lindblad time evolution in the
