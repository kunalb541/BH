...
particle loss, and direct bond control. The central finding is that the operative selector is
\emph{intervention-relative}---$\Fi$ for number-conserving transport handles (dephasing,
detuning), local occupation for number-changing loss---a structural point we return to in the
Discussion. All results are exact (no Trotter,
...
$(J/U,\tau)$ conditions.}
\label{tab:T1}
\input{outputs/paper_v2/tables/T1_predictor.tex}
\end{table}
...
regimes.}
\label{tab:T2}
\input{outputs/paper_v2/tables/T2_nondiscrim.tex}
\end{table}
...
disorder realizations where $\Fi$ and geometry select \emph{disjoint} sites (overlap $0$) in the
pocket. Handle percentiles are top-$\Fi$ vs.\ geometric-centre selectors.}
\label{tab:T3}
\input{outputs/paper_v2/tables/T3_geometry.tex}
\end{table*}
...
sites, so the operative variable is occupation. This is not a foregone conclusion: the competing
transport hypothesis---that high-current sites sustain \emph{more} loss through current
replenishment---is present but subdominant to raw occupation (Table~\ref{tab:T4}, where occupation
out-predicts current and $\Fi$), and $\Fi$'s residual loss-predictive power is only its clean-chain
correlation with $\langle\nop_i\rangle$. The boundary between the two selectors is thus
...
\label{tab:T4}
\input{outputs/paper_v2/tables/T4_loss.tex}
\end{table}
...
\label{tab:T5}
\input{outputs/paper_v2/tables/T5_bond.tex}
\end{table}
...
\label{tab:T6}
\input{outputs/paper_v2/tables/T6_map.tex}
\end{table}
